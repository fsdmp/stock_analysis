...
一个面向A股市场（主板、创业板、科创板）的本地化股票数据分析与可视化平台。支持沪市60xxxx（主板）、深市00xxxx（主板）、300xxx（创业板）、688xxx（科创板）。基于 Flask + ECharts 构建，集成多因子支撑/阻力识别、交易信号检测与降噪过滤算法，提供日K线与分时级别的交互式图表。
...
| 短线评分 | 12 维度趋势上下文评分引擎（含价格位置维度），tanh 非线性归一化，输出 0~100 综合评分与操作建议 |
...
| 回测系统 | 4 种回测模式 + 资金池模拟（初始资金/最大持仓/选股策略），支持多进程并行 |
| 自选股 | 分组管理自选股，支持批量添加、跨组移动、快速评分 |
| 交互式图表 | ECharts 四面板（K线+支撑阻力、成交量、MACD、KDJ），支持缩放/拖拽，移动端自适应 |
| 分时数据 | 支持 5 分钟 / 15 分钟级别分时K线，双击日K查看对应日期分时 |
| 股价对比 | 多股票多时段对比分析：涨跌幅、最大涨幅、最大回撤、波动率、夏普比率、振幅等 16 项指标 |
...
│   ├── scoring.py                 # 短线评分引擎（12维度趋势上下文评分，含价格位置）
...
│   └── __init__.py                # 模块入口
├── web/                           # Web 可视化
│   ├── app.py                     # Flask 后端（页面路由 + REST API + 资金池模拟）
│   └── static/
│       ├── home.html              # 首页（股票列表 + 搜索 + 更新）
│       ├── detail.html            # K线详情页（四面板图表 + 分时 + 移动端自适应）
│       ├── login.html             # 登录页
│       ├── recommend.html         # 推荐页（全量扫描结果展示 + 移动端适配）
│       ├── watchlist.html         # 自选股页面（分组管理 + 批量操作 + 移动端适配）
│       ├── compare.html           # 股价对比页面（多股票多时段对比分析 + 可视化图表）
│       └── strategy.html          # 策略管理页面（创建/编辑/扫描 + 资金模拟 + 移动端适配）
├── strategies/                    # 策略定义目录
...
├── backtest_signal.py             # 信号回测（基于评分的模拟交易 + 资金池模拟）
...
换手率直接从 baostock 数据源获取，反映股票交易的活跃程度。在评分系统中作为独立维度（权重 4）参与评分。
...
| 中长线权重过高 | 均线形态与动量等权 | **数据驱动权重**：基于 Pearson 相关性分析，正相关因子提权 |
| 评分聚集在 50 附近 | 线性归一化 + 多空抵消 | **tanh 非线性归一化**：放大信号差异 |

### 趋势检测

所有 12 个评分维度都依赖一个预计算的趋势状态（`_detect_trend`），使用 3-10 日短周期窗口：
...
> **v2 更新**: 新增"价格位置"维度（区分度最高的因子），所有维度权重基于数据驱动的相关性分析重新校准。

#### 1. 价格位置（权重 18）

**数据驱动核心维度**，基于 21 只股票回测验证，区分度排序最高的因子（r=+0.281 最强正相关）。

| 子因子 | 区分度 | 评分规则 |
|--------|--------|----------|
| 10日区间位置 | 1.00 | 触及10日高点(≥99%) → -7；贴近10日高点(>95%) → -4；最佳区间(60~85%) → +3 |
| 距5日高点距离 | 0.99 | 紧贴5日高点(>-0.5%) → -3；适度回调(-5%~-2%) → +3（最佳买点区间） |
| 5日振幅活跃度 | 0.70 | 振幅>22% → +2（活跃）；振幅<8% → -2（死水） |

> **v3 更新**: 上升趋势中价格位置解读发生变化：10日区间>85%视为正常趋势延伸（+1），贴近10日高点(>95%)仅轻微扣分（-1）；创5日新高在上升趋势中视为正面信号（+1）。

**核心规律**: 非上升趋势中，价格不能紧贴近期最高点，需要有回调空间；上升趋势中，高位是正常的趋势延伸。活跃度（振幅）高的票更容易延续上涨。
...
| 3 日涨 3~5% | 上升 | +3 | 健康动量 |
| 3 日涨 5~10% | 上升 | +4 | 强动量 |
| 3 日涨 10~15% | 上升 | +3 | 主升浪动量 |
| 3 日涨 15~20% | 上升 | +1 | 强势但偏高 |
| 3 日涨 > 20% | 上升 | -2 | 短期过热 |
...
**涨停炸板因子**（最高价触及涨停价但收盘未封住）：

| 场景 | 回落幅度 | 趋势 | 得分 | 说明 |
|------|----------|------|------|------|
| 炸板大幅回落 | >5% | 任意 | -6 | 空头力量极强 |
| 炸板回落 | 3~5% | 任意 | -4 | 抛压较大 |
| 炸板小幅回落 | 1~3% | 上升 | -1 | 上升趋势试盘 |
| 炸板小幅回落 | 1~3% | 非上升 | -3 | 上方压力大 |
| 触板未封 | <1% | 上升 | +1 | 强势试探 |
| 触板未封 | <1% | 非上升 | -2 | 上方压力 |
| 近5日≥2次炸板 | — | 任意 | -2 | 上方压力持续 |
| 近期炸板 | — | 下降 | -1 | 反弹失败 |
...
| 高换手上涨 | 10%~20% | >3% | 非上升 | +2 | 反弹 |
| 高换手下跌 | 10%~20% | <-3% | — | -4 | 资金出逃 |
| 缩量涨停 | <5% | 涨停 | — | +4 | 一致性极强 |
| 巨量涨停 | >30% | 涨停 | — | -3 | 严重分歧 |
| 换手暴增 | 3倍+ | 涨 | — | +3 | 资金涌入 |
| 换手递增价涨 | 递增 | 涨 | 上升 | +3 | 主升浪确认 |
| 连续3日高换手 | >10%×3日 | — | 上升 | -2 | 获利盘积累 |
| 连续3日高换手 | >10%×3日 | — | 下降 | -3 | 持续出货 |

**特殊处理**：
- 涨停板换手分析：缩量<5%最佳，>30%为严重分歧
- 跌停板换手分析：放量>15%恐慌，缩量<3%封死
- 连续高换手（>10% x 3日）：获利盘积累，上升趋势需警惕
- **涨停炸板换手分析**：炸板日的高换手 = 涨停价附近大量换手

| 炸板换手场景 | 换手率 | 得分 | 说明 |
|-------------|--------|------|------|
| 炸板巨量换手 | >活跃涨停阈值 | -4 | 涨停价出货 |
| 炸板放量换手 | 适度~活跃区间 | -2 | 分歧较大 |
| 炸板缩量换手 | <缩量涨停阈值 | -1 | 跟风不足 |
| 炸板适度换手 | 其他 | -1 | 未封板 |
...
| 检查项 | 说明 |
|--------|------|
| 信号一致性 | >= 6 维度同向 + 总能量 > 15 → 共振加成（看多+3/+1，看空-3/-1） |
| 信号混乱 | 正/负维度均 ≤ 2 → -1 |
| 趋势确认加成 | 趋势方向与得分方向一致 → ±2 |
| 高位放量反诱多 | 偏离 MA20 > 8% + 放量 + 下降趋势 → -5，非下降 → -3 |
| 低位急跌反诱空 | 偏离 MA20 < -8% + 上升趋势 → +4，非上升 → +2 |
| 大涨后回调 (5日涨>10%) | 上升趋势中 → +1 (买点)，下降趋势中 → -3 (危险) |
| 大跌后反弹 (5日跌<-10%) | 上升趋势中 → +3 (抄底)，下降趋势中 → 0 (不可信) |

#### 6. 主力行为（权重 9）

K 线形态 + 量价异常 + 尾盘操纵检测。

> **v2 更新**: 影线分析改为数据驱动模型，基于上影线/下影线比率（而非绝对长度）评估攻击意愿与抛压；十字星区分高位/普通位置。

**影线分析（比率模型）：**

| 形态 | 趋势 | 得分 | 说明 |
|------|------|------|------|
| 明显上影线(比率≥0.35) | 上升 | +3 | 上攻试探 |
| 明显上影线(比率≥0.35) | 非上升 | +1 | 有攻击意愿 |
| 上影线(比率≥0.25) | 任意 | +1 | 有攻击意愿 |
| 几乎无上影(比率<0.10) + 下影>0.25 | — | -1 | 上攻乏力 |
| 长下影线(比率≥0.50) | — | -3 | 盘中抛压重 |
| 下影线偏长(比率≥0.35) | — | -1 | 有一定抛压 |
| 短下影(比率<0.15) + 上影≥0.25 | — | +1 | 抛压轻 |

**十字星（高位/普通区分）：**

| 形态 | 条件 | 得分 | 说明 |
|------|------|------|------|
| 高位十字星 | 上升 + 10日区间>85% | -4 | 见顶信号 |
| 十字星 | 上升（非高位） | -2 | 上升受阻 |
| 十字星 | 下降 | +2 | 下跌减速 |
| 十字星 | 震荡 | -1 | 方向不明 |
...
| 形态 | 条件 | 得分 | 说明 |
|------|------|------|------|
| 炸板长上影 | 上影线比率>60% | -4 | 主力出货 |
| 炸板上影 | 上影线比率>40% | -2 | 抛压明显 |
| 炸板中上影 | 上影线比率>25% | -1 | 有抛压 |
| 炸板收阴 | 收盘<开盘 | -2 | 多头溃败（叠加） |
...
| DIF > DEA | — | +3 | 多头排列 |
| DIF < DEA | — | -3 | 空头排列 |
| 零轴上方 (DIF>0, DEA>0) | — | +2 | 多头区域 |
| 零轴下方 (DIF<0, DEA<0) | — | -2 | 空头区域 |
| 红柱放大 | 任意 | +3 | 多头加速 |
| 红柱缩短 | 上升 | +1 | 上升整理，不算看空 |
| 红柱缩短 | 非上升 | -1 | 多头动能衰减 |
| 绿柱放大 | 任意 | -3 | 空头加速 |
...
> **v2 更新**: 基于 21 只股票回测的 Pearson 相关性分析重新校准权重。正相关因子提权，负相关因子降权。

| 维度 | 权重 | 相关系数 r | 设计理由 |
|------|------|-----------|----------|
| 价格位置 | **18** | +0.281 | 最强正相关，区分度最高 |
| MA 趋势 | **10** | +0.187 | 正相关，方向判断 + VWMA 反诱多 |
| 主力行为 | **9** | +0.017 | 识别主力操纵痕迹 |
| 量价配合 | **7** | -0.225 | 负相关降权，验证价格运动 |
| 支撑压力 | **5** | +0.067 | 近期关键价位参考 |
| MACD 动能 | **5** | -0.266 | 负相关大幅降权，滞后指标 |
| 短期动量 | **10** | -0.127 | 略降，1-3 天核心因素 |
| 换手率 | **4** | -0.292 | 最强负相关，大幅降权 |
| KDJ 状态 | **3** | -0.125 | 敏感短期震荡指标，低权重 |
| 趋势确认 | **2** | — | Meta 层面信号一致性检验 |
| 均线形态 | **2** | — | 前兆指标，非行动指标 |
| 背离信号 | **2** | — | 小概率但高价值信号 |
...
| 涨停炸板降温（大幅回落） | 当日最高触及涨停价但收盘未封住，回落>5% | 评分上限 60（空头极强） |
| 涨停炸板降温（中等回落） | 炸板回落 3~5% | 评分上限 65（抛压较大） |
| 涨停炸板降温（小幅回落） | 炸板回落 1~3% | 评分上限 70（温和降温） |
| 涨停炸板降温（极小回落） | 炸板回落 <1% | 评分上限 74（轻微降温） |
| 非上升趋势过热 | 非上升 + 当日涨 >5% 且偏离 MA20 > 8% | 评分上限 68 |
| 非上升趋势轻度过热 | 非上升 + 当日涨 >3% 且偏离 MA20 > 5% | 评分上限 75 |

> **v3 更新**: 上升趋势中不再执行过热降温——上升通道中的上涨偏离 MA20 是正常的趋势延伸，不应被惩罚。降温规则仅在非上升趋势（震荡/下降）中生效，且阈值从"偏离3%→封顶72"放宽到"偏离5%→封顶75"。
...
```
综合评分76分。短期趋势: 上升；主要支撑: 价格位置(10日区间适中72%; 距5日高点-3.2%); 短期动量(上涨+1.8%; 3日涨+4.2%(健康动量))；主要风险: MACD动能(DIF<DEA; 零轴下方)
```
  ...
  "dimensions": [
    {
      "name": "价格位置",
      "score": 5,
      "detail": "10日区间适中(72%); 距5日高点-3.2%(适度回调)",
      "weight": 18
    },
    {
      "name": "短期动量",
      "score": 7,
      "detail": "上涨+1.8%; 3日涨+4.2%(健康动量)",
      "weight": 10
    },
    {
      "name": "MA趋势",
      "score": 4,
      "detail": "多头排列; MA5>MA10",
      "weight": 10
    }
  ],
  "summary": "综合评分76分。短期趋势: 上升；主要支撑: 价格位置(...); 主要风险: ..."
}
```
...
- 最佳/最差交易 Top 5
- 盈亏比和理论复合收益率
- **资金池模拟**（10万初始，最多同时持仓3只，真实收益/最大回撤/真实年化）
...
通过 Web API 触发回测，支持在浏览器中查看结果。新增资金池模拟功能，模拟真实资金约束下的交易表现。

**资金池模拟参数：**

| 参数 | 类型 | 默认 | 说明 |
|------|------|------|------|
| initial_capital | float | 100000 | 初始资金（元） |
| max_positions | int | 3 | 最大同时持仓数量 |
| pick_strategy | string | "fifo" | 同日多信号选股策略 |
...
**模拟规则：**
- 初始资金平均分配到 max_positions 个持仓槽位
- 每个槽位同时最多持有 1 只股票
- 买入信号按时间顺序处理，仅有空闲槽位时才执行
- 同日多个买入信号超出空闲槽位时，按 pick_strategy 决定优先级
- 卖出后释放槽位，资金回收再利用
- 输出真实总收益、最大回撤、实际成交笔数

**报告展示：**
- 理论复合收益率 vs 资金模拟真实收益
- 资金池概览（初始/最终资金/总收益/最大回撤/成交数）
- 收益分布、月度统计、最佳/最差交易
...
股价对比功能支持选择多只股票和多个时间段，横向对比各股票在不同时段的关键指标。
...
| 功能 | 说明 |
|------|------|
| 多股票选择 | 搜索添加、手动输入代码、从自选股分组批量导入 |
| 多时段选择 | 9 个预设时段（近1周~近3年、今年）+ 自定义日期范围 |
| 对比表格 | 按时间段分组展示全部 16 项指标，直观对比 |
| 可视化图表 | ECharts 柱状图：涨跌幅对比、最大涨幅 vs 最大回撤 |
| 摘要卡片 | 每只股票独立卡片，关键指标一目了然 |

**对比指标（共 16 项）：**

| 指标 | 说明 |
|------|------|
| 涨跌幅 | 期初开盘价到期末收盘价的变动百分比 |
| 最大涨幅 | 区间内收盘价相对期初价的最大涨幅 |
| 最大回撤 | 区间内从任一高点到后续低点的最大跌幅 |
| 振幅 | (最高价 - 最低价) / 期初价 |
| 波动率 | 日收益率标准差 × √交易日数（年化） |
| 夏普比率 | 年化超额收益 / 年化波动率（无风险利率=0） |
| 均换手率 | 区间内日均换手率 |
| 交易日 | 区间内实际交易天数 |
| 期初价 / 期末价 | 区间首日开盘价 / 末日收盘价 |
| 最高价 / 最低价 | 区间内最高/最低价格 |
| 上涨天数 / 下跌天数 | 日涨幅为正/为负的交易日数 |
| 最大连涨 / 最大连跌 | 最大连续上涨/下跌天数 |
...
curl -X POST http://localhost:8888/api/compare \
  ...
  -d '{
    ...
    "periods": [
      ...
      {"label": "今年", "start": "2026-01-01", "end": "2026-05-28"}
    ]
  }'
```
...
|------|------|------|
| GET | `/compare` | 股价对比页面 |
| POST | `/api/compare` | 执行对比分析，返回各股票各时段指标 |
...
| GET | `/api/score/<code>` | 短线评分（12维度趋势上下文评分） |
