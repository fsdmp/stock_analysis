...
| 全量扫描 | 后台扫描全部股票，按评分筛选，支持最小分数和价格过滤 |
...
│   ├── indicators.py              # 技术指标计算（MA/VWMA/BB/MACD/KDJ/OBV/量比）
...
| 上涨 1~3% | 下降 | 0 | 下降趋势反弹 = 不可信 |
| 3 日涨 > 10% | 上升 | +2 | 强势动量，顺势 |
| 3 日跌 < -10% | 上升 | +2 | 超跌反弹机会 |
| 3 日涨 > 10% | 下降 | -3 | 无量反弹过热 |
| 连阳 ≥ 3 日 | 上升 | +3 | 趋势配合加速 |
...
**这是趋势上下文改变最大的一维**。传统分析中 KDJ 超买 = 看空，但在短线趋势交易中这是错误的。
...
| 维度 | 权重 | 设计理由 |
|------|------|----------|
| 短期动量 | **20** | 1-3 天决策最核心因素 |
| 换手率 | **15** | 超短线核心维度，反映资金活跃度 |
| 量价配合 | **15** | 验证一切价格运动的真伪 |
| KDJ 状态 | **13** | 敏感短期震荡指标 |
| 主力行为 | **10** | 识别主力操纵痕迹 |
| MA 趋势 | 7 | 方向判断 + VWMA 反诱多 |
| 支撑压力 | 7 | 近期关键价位参考 |
| MACD 动能 | 5 | 有用但滞后 |
| 背离信号 | 4 | 小概率但高价值信号 |
| 均线形态 | 2 | 前兆指标，非行动指标 |
...
normalized = 50 + 50 × tanh(3.5 × raw_ratio)
```
...
实际运行 30 只股票的评分分布（标准差 22，范围 7~85）：
...
```
综合评分76分。短期趋势: 上升；主要支撑: 短期动量(上涨+1.8%; 连阳3日(趋势配合); 3日涨+10.4%(强势))；主要风险: MACD动能(DIF<DEA; 零轴下方)
```
  ...
  "dimensions": [
    {
      "name": "短期动量",
      "score": 9,
      "detail": "上涨+1.8%; 连阳3日(趋势配合); 3日涨+10.4%(强势)",
      "weight": 20
    },
    {
      "name": "量价配合",
